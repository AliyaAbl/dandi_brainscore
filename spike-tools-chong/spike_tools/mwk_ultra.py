...
parser.add_argument('--eyedata', type=int, default=0)
...
# Record eye data to mworks output
eyedata = args.eyedata
...
def get_events(filepath, names, time_range=[0, np.inf]):
    ...
    with MWK2Reader(filepath) as event_file:
        ...
        total_iterations = 0
        #Find total number of events in event file
        for _ in event_file:
            total_iterations += 1

        progress_bar = tqdm(total=total_iterations, desc='Getting Events')
...
def dump_events(filename, photodiode_file, sample_on_file, artefact_diode_flag = False, reverse_flag=False, skip_num=None):
    ...
    if args.date or date:
        print('Date exists')
        baseDir = os.path.join('../projects/', config['Experiment Information']['name'], 'monkeys', config['Experiment Information']['monkey'])
        ...
        # [::-1] reverses the list
        intanRawDirList = [i for i in os.listdir(intanRawDir) if date+'_' in i]
        intanRawDir = os.path.join(intanRawDir, intanRawDirList[args.session_num])
        filenameList = [i for i in os.listdir(mworksRawDir) if date in i]
...
#     event_file.open()
    ...
    # event_file.close()
    print(data[data.name == 'current_vid']['data'].values)
    ###########################################################################
    ...
    output = {
        ...
        #'current_seq': data[data.name == 'current_seq']['data'].values,
        #'current_video': data[data.name == 'current_vid']['data'].values
    }
    ...
    if artefact_diode_flag:
        ...
        peaks_mean = np.mean(v[peaks])
        peaks = np.asarray([p for p in peaks if v[p] > peaks_mean])  # Apply threshold
        print('Threshold: ', peaks_mean)
    ...
    meanms = np.mean(np.abs(photodiode_on - samp_on)) / 1000.
    stdms = np.std(np.abs(photodiode_on - samp_on)) / 1000.
    ...
    for i, x in enumerate(photodiode_on - samp_on):
        if (x/1000.)-meanms > 2*stdms or (x/1000.)-meanms < -2*stdms:
    ...
    print(num_bad_samples,meanms,stdms)
        ###########################################################################
        # Get eye data
        ###########################################################################
    if (eyedata):
        begin_time = np.array(data[(data['name']== 'stimulus_presented')]['time'])[0]
        data['time'] = data['time'] - begin_time
        eye_h, eye_v, eye_time = [], [], []
        pupil_size, pupil_time = [], []
        for t in tqdm(stimulus_presented_df.time.values, desc='Extracting Eye Position Data'):
            t1 = int(t - 50 * 1000.)  # Start time (ms)
            t2 = int(t + (output['stim_on_time_ms'] + 50 * 1000.))  # Stop time (ms)
            h = np.array(data[(data['name'] == 'eye_h') & (data['time'] >= t1) & (data['time'] <= t2)]['data'])
            v = np.array(data[(data['name'] == 'eye_v') & (data['time'] >= t1) & (data['time'] <= t2)]['data'])
            time = np.array(data[(data['name'] == 'eye_v') & (data['time'] >= t1) & (data['time'] <= t2)]['time'])
            time = (time - t) / 1000
            assert len(h) == len(v)
            ...
            # p = np.array(data[(data['name'] == 'pupil_size_r') & (data['time'] >= t1) & (data['time'] <= t2)]['data'])
            # p_time = np.array(data[(data['name'] == 'pupil_size_r') & (data['time'] >= t1) & (data['time'] <= t2)]['time'])
            # p_time = (p_time - t) / 1000
